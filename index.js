...
function validateClientNumber(n) { return /^\d{8}$/.test(String(n||'').trim()); }
...
function proto() { return 'SN-' + Math.random().toString(36).substring(2,6).toUpperCase() + '-' + Date.now().toString().slice(-6); }
...
app.post('/webhook', (req, res) => {
  ...
  if (intent === 'intent.abrir_chamado') {
    let { problema, dispositivo, endereco, cidade, plano, numero_cliente, janela_data, janela_hora, preferencia_contato, nome, contato } = p;
    ...
    const contactOK = phone || email ? true : false;

    if (!validateClientNumber(numero_cliente)) return dfResponse('O número de cliente deve ter 8 dígitos. Pode confirmar?');
    if (!contactOK) return dfResponse('Preciso de um telefone com DDD ou e‑mail válido para prosseguir. Pode me informar?');
    ...
    tickets[protocolo] = {
      protocolo, problema, dispositivo, endereco, cidade, plano, numero_cliente,
      preferencia_contato: phone ? phone : email, nome, whenISO,
      etapa: 'Agendado', previsao: whenISO
    };
    ...
    return dfResponse(
      ...
      [{ name: `${body.session}/contexts/ctx_chamado_aberto`, lifespanCount: 5, parameters: { protocolo, previsao: whenISO } }]
    );
  }
  ...
  if (intent === 'intent.status_chamado') {
    ...
    if (protocolo && tickets[protocolo]) {
      ...
      if (ctx?.parameters?.protocolo && tickets[ctx.parameters.protocolo]) candidate = tickets[ctx.parameters.protocolo];
    }
    if (!candidate) return dfResponse('Não encontrei seu chamado. Informe o protocolo ou seu telefone/e‑mail cadastrado para eu localizar.');
