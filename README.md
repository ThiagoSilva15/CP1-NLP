...
No Render, crie um Web Service → conecte seu repositório.
Build Command: npm install
Start Command: npm start
Após deploy, copie a URL pública (ex.: https://cp2-webhook.onrender.com).
Configurar no Dialogflow (Fulfillment)
Dialogflow ES → Fulfillment → Enable webhook:
URL: https://SEU-APP.onrender.com/webhook
Em Intents → abra os intents que exigem lógica (ex.: intent.abrir_chamado e intent.status_chamado):
Marque Use webhook (Fulfillment).
Teste no Simulator do Dialogflow:
...
Abrir chamado curl -X POST https://SEU-APP.onrender.com/webhook -H 'Content-Type: application/json' -d '{ "session": "projects/demo/agent/sessions/abc", "queryResult": { "intent": { "displayName": "intent.abrir_chamado" }, "parameters": { "problema": "Sem conexão", "endereco": "Rua das Flores, 123", "cidade": "São Paulo", "plano": "300 Mega", "numero_cliente": "CLI-778899", "janela_data": "2025-10-12", "janela_hora": "14:00" } } }'

Status do chamado curl -X POST https://SEU-APP.onrender.com/webhook -H 'Content-Type: application/json' -d '{ "session": "projects/demo/agent/sessions/abc", "queryResult": { "intent": { "displayName": "intent.status_chamado" }, "parameters": { "protocolo": "SN-XXXXXX" } } }'

Baixar evidência dos dados curl https://SEU-APP.onrender.com/data/download

Roteiro do vídeo (≤ 3 min)
Contexto (10s) — "Este é o SuporteNet integrado a um webhook no Render."
Dialogflow (45s) — Mostrar:
Fulfillment habilitado e URL do webhook.
Em intent.abrir_chamado, checkbox Use webhook marcado.
Teste (60s) — No simulator ou Telegram:
Abrir chamado → receber protocolo.
Consultar status_chamado com o protocolo.
Evidência backend (30s) — Abrir https://SEU-APP.onrender.com/data/download e mostrar o JSON com o chamado gravado.
Encerramento (10s) — "Integração concluída: validação de parâmetros, geração de protocolo e retorno em fulfillmentMessages."
Estrutura
cp2-webhook/ ├─ src/ │ ├─ index.js │ ├─ dialogflow.js │ └─ store.js ├─ data/ │ └─ seeds/faq.json ├─ package.json └─ README.md
